...
# BiliQueue - Bilibili 网站单标签临时播放队列增强

在哔哩哔哩（Bilibili）的主页、搜索页和播放页分别提供一个标签级的临时播放队列，方便用户将自己感兴趣的视频快速加入到临时播放列表来观看。

# BiliQueue（哔哩队列）介绍

## 项目简介

BiliQueue 是一个面向 Bilibili 网站的 Chrome 扩展
参考 YouTube 的使用体验, 主打“单标签页、零打扰”的临时播放队列，
通过在视频卡片上注入轻量操作按钮，快速将页面内感兴趣的视频加入到临时播放列表，而不必加入重量级的收藏夹或者稍后再看列表。
